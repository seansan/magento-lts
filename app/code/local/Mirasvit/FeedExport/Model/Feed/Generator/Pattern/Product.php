    ...
    extends Mirasvit_FeedExport_Model_Feed_Generator_Pattern
{
    ...
    public function getValue($pattern, $product)
    {
        ...
        } elseif ($pattern['type'] == 'admin') {
            $product = $this->getAdminProduct($product);
                ...
                if ($type == 'configurable') {
                    $childIds = Mage::getModel('catalog/product_type_configurable')
                        ...
                        foreach ($childIds[0] as $id) {
                            ...
                            if ($child->getData('gr_ean')) {
                                $value = $child->getData('gr_ean');
                                break;
                            }
                        }

                } else {
                    $value = $this->getParentProduct($product)->getData('gr_ean');
                }
        ...
        public function getParentProduct(Varien_Object $product, $isOnlyParent = false)
    {
        ...
        if ($parentId > 0) {
            if (!isset(self::$_parentProductsCache[$parentId])) {
                self::$_parentProductsCache[$parentId] = Mage::getModel('catalog/product')->load($parentId);
            }
        }
